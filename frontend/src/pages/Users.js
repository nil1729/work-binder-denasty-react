...
function Users({ addAlert }) {
	...
	const [receive_players, set_receive_players] = useState(Array.from({ length: 6 }));
	...
	const selectPlayer = (player, player_type, selectedIndex) => {
		if (player_type === 'giver') {
			...
			set_give_value(Number(parseFloat(give_value) + parseFloat(player.value)).toFixed(2));
			...
			if (filledCount % 3 === 0 && filledCount >= 6 && newPlayerSet.length <= filledCount) {
			...
			set_receive_value(Number(parseFloat(receive_value) + parseFloat(player.value)).toFixed(2));
			setSelectedPlayers([...selectedPlayers, player]);
	...
	const removePlayer = (player, player_type, selectedIndex) => {
		if (player_type === 'giver') {
			...
			set_give_value(Number(parseFloat(give_value) - parseFloat(player.value)).toFixed(2));
			setSelectedPlayers(selectedPlayers.filter((it) => it.rank !== player.rank));
		} else {
			set_receive_players(
	...
	const editPlayer = (player, player_type, selectedIndex, old_player) => {
		if (player_type === 'giver') {
			...
			set_give_value(
				Number(
					parseFloat(give_value) + parseFloat(player.value) - parseFloat(old_player.value)
				).toFixed(2)
			);
			setSelectedPlayers([...selectedPlayers, player].filter((it) => it.rank !== old_player.rank));
		} else {
			set_receive_players(
				receive_players.map((it, index) => {
					if (index === selectedIndex) return player;
			...
			set_receive_value(
				Number(
					parseFloat(receive_value) + parseFloat(player.value) - parseFloat(old_player.value)
				).toFixed(2)
			);
			setSelectedPlayers([...selectedPlayers, player].filter((it) => it.rank !== old_player.rank));
		}
								...
								<UserItem className={styles.user__item} key={index}>
									...
									<div className={styles.bottom__container}>
										<div className={styles.badge__container}>
											...
											<div className={styles.btn__tag}>{item && item.position}</div>
										</div>
										<div
